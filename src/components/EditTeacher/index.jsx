...
function EditTeacher() {
  ...
  const [selectedDivisions, setSelectedDivisions] = useState([]); // 用于存储选中的division
  ...
  const handleMoveRightClick = () => {
    // 在这里获取选中的division并更新到 selectedDivisions 状态
    // 假设你已经实现了获取选中的division的逻辑
    // 以下是一个示例：
    ...
    // 遍历复选框元素，如果选中则将对应division添加到数组中
    ...
    // 在这里将选中的division更新到右侧部分的输入框
    setInputValue(selectedNames.join(', ')); // 将division用逗号分隔显示
  };
                ...
                {DivisionsData.map((Division) => (
                  <tr key={Division.id}>
                    <td>
                      {/* 
                        渲染复选框
                        使用 data-Division-name 属性存储division
                      */}
                      <input
                        type="checkbox"
